...
class Cpab(object):
    ...
    def __init__(self, 
        ...
        if self.backend_name == 'numpy':
            ...
            #import torch 
            #from torch import functional as backend
    ...
    #%%
    def transform_grid(self, grid, theta, return_velocities=False):
        ...
        if return_velocities:
            transformed_grid, velocities = self.backend.transformer(grid, theta, self.params, return_velocities)
            return transformed_grid, velocities
        else:
    ...
    #%%
    def transform_data(self, data, theta, outsize, return_grid = False, return_velocities = False):
        ...
        if return_velocities:
            grid_t, velocities = self.transform_grid(grid, theta, return_velocities)
        else:
            grid_t= self.transform_grid(grid, theta)
        
        data_t = self.interpolate(data, grid_t, outsize)

        if return_grid and return_velocities:
            return data_t, grid_t, velocities
        if return_grid:
            return data_t, grid_t
        if return_velocities:
